...
check_prerequisites() {
    local required_files=(
        "scripts/extraction/pubmed_drug_indications.jl"
        "scripts/extraction/slurm_pubmed_drug_indications.jl"
        "approved_drugs_dict.jl"
    )
    
    # Check for either new or old mesh headings file
    local mesh_file_found=false
    for mesh_file in "mesh_semantic_headings.jl" "mesh_t047_headings.jl"; do
        if [[ -f "$mesh_file" ]]; then
    ...
    if [[ "$mesh_file_found" == "false" ]]; then
        log_error "Neither mesh_semantic_headings.jl nor mesh_t047_headings.jl found"
        return 1
    fi
...
build_julia_command() {
    local cmd="julia scripts/extraction/slurm_pubmed_drug_indications.jl"
