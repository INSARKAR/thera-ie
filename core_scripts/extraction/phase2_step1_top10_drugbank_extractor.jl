...
"""
THERA-IE Phase 2 Step 1: Top 10 DrugBank SNOMED Indication Extractor

This script processes the top 10 US prescribed drugs using Llama 3.2's analysis of DrugBank 
indication text to extract drug indications with direct SNOMED CT terminology output.

Key Features:
- Analyzes DrugBank indication text using LLM intelligence  
...
Date: 2025-08-02
Version: 1.0 (Top 10 DrugBank SNOMED CT Extraction)
"""
...
const PROMPT_FILE = "prompts/llm_prompt_drugbank_snomed_top10.txt"
const OUTPUT_DIR = "phase2_step1_drugbank_snomed"
const DRUGBANK_DIR = "phase1_step1_drugbank_extraction"
const TOP10_FILE = "top_10_drugs_us.txt"

# Data structures for SNOMED CT output
struct SnomedDrugBankIndication
    snomed_code::String
    snomed_term::String
    confidence::Float64
    evidence::Bool
...
struct SnomedDrugBankExtractionResult
    drug_name::String
    original_indication_text::String
    total_indications::Int
    confirmed_indications::Int
    snomed_indications::Vector{SnomedDrugBankIndication}
...
function load_drugbank_data(drug_name::String)
    """Load DrugBank indication data for a specific drug"""
    try
        drugbank_file = joinpath(DRUGBANK_DIR, "$(drug_name)_drugbank.json")
        if !isfile(drugbank_file)
            println("❌ DrugBank file not found: $drugbank_file")
            return nothing
        end
        
        json_content = read(drugbank_file, String)
        drug_data = JSON3.read(json_content)
        
        indication_text = string(get(drug_data, "drugbank_indication", ""))
        if isempty(indication_text)
            println("❌ No indication text found for $drug_name")
            return nothing
        end
        
        println("✓ Loaded DrugBank data for $drug_name ($(length(indication_text)) characters)")
        return indication_text
    catch e
        println("❌ Failed to load DrugBank data for $drug_name: $e")
        return nothing
    end
end

function load_drugbank_snomed_prompt_template()
    """Load the SNOMED CT DrugBank prompt template from file"""
    try
        ...
        println("❌ Failed to load SNOMED CT DrugBank prompt template: $e")
        return nothing
    end
end
...
function parse_snomed_drugbank_response(llm_response::AbstractString)
    """Parse LLM response to extract SNOMED indications for DrugBank approach"""
    indications = SnomedDrugBankIndication[]
    ...
    for block in blocks
        try
            ...
            if !isempty(snomed_code) && !isempty(snomed_term)
                indication = SnomedDrugBankIndication(
                    snomed_code,
                    snomed_term,
                    confidence,
                    evidence,
    ...
    unique_indications = SnomedDrugBankIndication[]
...
function process_drug_snomed_drugbank(drug_name::String, indication_text::String, prompt_template::String, ollama_host::String)
    """Process a single drug's DrugBank indication text using SNOMED CT extraction"""
    
    start_time = time()
    
    # Format the prompt with drug name and DrugBank indication text
    prompt = replace(prompt_template, "{DRUG_NAME}" => drug_name, count=10)
    prompt = replace(prompt, "{INDICATION_TEXT}" => indication_text, count=10)
    ...
    if contains(llm_response, "I can't provide medical advice") || 
       ...
       contains(llm_response, "medical advice")
        ...
        return SnomedDrugBankExtractionResult(
            drug_name,
            indication_text,
            0,
            0,
            SnomedDrugBankIndication[],
            processing_time,
            string(now()),
            MODEL_NAME,
            "drugbank_snomed_extraction",
            llm_response
        )
    end
    
    # Parse response for SNOMED indications
    snomed_indications = parse_snomed_drugbank_response(llm_response)
    ...
    # Create result
    processing_time = time() - start_time
    result = SnomedDrugBankExtractionResult(
        drug_name,
        indication_text,
...
function save_snomed_drugbank_results(result::SnomedDrugBankExtractionResult)
    """Save SNOMED CT DrugBank results to JSON file with consistent format"""
    safe_filename = sanitize_filename(result.drug_name)
    output_file = joinpath(OUTPUT_DIR, "$(safe_filename)_drugbank_snomed.json")
    ...
    try
        ...
        output_data = Dict(
            ...
            "metadata" => Dict(
                "original_indication_text" => result.original_indication_text,
            ...
            "debug_info" => Dict(
                ...
                "indication_text_length" => length(result.original_indication_text),
                ...
                "processing_notes" => result.total_indications > 0 ? "Successful SNOMED CT extraction from DrugBank text" : "No indications extracted"
            )
        )
...
function main()
    """Main function to process top 10 drugs with DrugBank SNOMED CT extraction"""
    
    println("=== THERA-IE Phase 2 Step 1: Top 10 DrugBank SNOMED Extraction ===")
    ...
    # Load SNOMED CT prompt template
    prompt_template = load_drugbank_snomed_prompt_template()
    ...
    for (idx, drug_name) in enumerate(drugs_to_process)
        ...
        # Load DrugBank indication text
        indication_text = load_drugbank_data(drug_name)
        if indication_text === nothing
            failed_count += 1
            continue
        end
        ...
        # Process the drug
        result = process_drug_snomed_drugbank(drug_name, indication_text, prompt_template, ollama_host)
        if result === nothing
            failed_count += 1
            continue
        end
        ...
        # Save results
        if save_snomed_drugbank_results(result)
            processed_count += 1
    ...
    println("")
    println("=== PHASE 2 STEP 1 COMPLETE ===")
